...
MergeW2::MergeW2( sc_module_name n ): sc_module( n )
{
#ifndef NATIVE_SYSTEMC
...
//void MergeW2::merge(int first, int second, int width)
//{
//}

void MergeW2::do_merge() {
	{
#ifndef NATIVE_SYSTEMC
	...
	while (true) {
		...
		{
		for (int  first = 0; first < 16; first += 2 * 2) {
			HLS_UNROLL_LOOP(ON, "w2unroll");
			int second = first + 2;
			int end = second + 2 - 1;
			int l1 = first, l2 = second;
			int index = first;
			//HLS_CONSTRAIN_LATENCY (0, 10, "Merge2W_constrain");
		
			for (int i = 0; i < 2 * 2; i++) {
				//HLS_UNROLL_LOOP(ON, "w2");
				HLS_PIPELINE_LOOP(HARD_STALL, 1, "w2");
